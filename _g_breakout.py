...
class Ball(object):
    ...
    def __init__(self, x, y, x_speed, y_speed, display, width=4, height=4,
                 frozen=False):
        self.x = x
        self.y = y
        self.x2 = x + width - 1
    ...
    def clear(self):
        """Clear ball."""
        self.display.fill_rect(self.x, self.y, self.width, self.height, st7789.BLACK)

    def clear_previous(self):
        """Clear prevous ball position."""
        self.display.fill_rect(self.prev_x, self.prev_y,
                                self.width, self.height, st7789.BLACK)

    def draw(self):
        """Draw ball."""
        self.clear_previous()
        self.display.fill_rect( self.x, self.y,
                                 self.width, self.height,st7789.WHITE)
    ...
    def set_position(self, paddle_x, paddle_y, paddle_x2, paddle_center):
        ...
        if self.y < 16:
            ...
            bounced = True
        if self.x + self.width >= 135:
            self.x = 135 - self.width
...
class Brick(object):
    ...
    def __init__(self, x, y, color, display, width=16, height=6):
        """Initialize brick.
    ...
    def draw(self):
        """Draw brick."""
        self.display.rect(self.x, self.y, self.width, self.height, st7789.WHITE)
...
class Life(object):
    ...
    def __init__(self, index, display, width=4, height=6):
        ...
        margin = 5
        self.display = display
        self.x = self.display.width() - (index * (width + margin))
        self.y = 0
        self.width = width
        self.height = height
        self.draw()
    ...
    def draw(self):
        """Draw life."""
        self.display.fill_rect(self.x, self.y, self.width, self.height, st7789.YELLOW)
...
class Paddle(object):
    ...
    def draw(self):
        """Draw paddle."""
        self.display.fill_rect(self.x, self.y,self.width, self.height,st7789.WHITE)
        ...
        if new_x != self.x :  # Check if paddle moved
            ...
            if x > prev_x:
                self.display.fill_rect(prev_x, self.y,
                                        x - prev_x, self.height, st7789.BLACK)
            else:
...
class Score(object):
    ...
    def __init__(self, display):
        ...
        margin = 5
        self.display = display
        self.display.text(font, 'S:', margin, 0, st7789.WHITE)
        self.x = 20 + margin
        self.y = 0
        self.value = 0
        self.draw()
    ...
    def draw(self):
        """Draw score value."""
        self.display.fill_rect(self.x, self.y, 20, 8, st7789.BLACK)
        self.display.text(font, str(self.value), self.x, self.y,st7789.WHITE)

    def game_over(self):
        """Display game_over."""
        self.display.text(font, 'GAME OVER', (self.display.width() // 2) - 30,
                               int(self.display.height() / 1.5), st7789.WHITE)
...
def load_level(level, display) :
    ...
    bricks = []
    for row in range(18, 20 + 12 * level , 10):
        brick_color = 1
        for col in range(8, 112, 20 ):
...
while not exitGame :
    ...
    while True:
        # g.display.fill(0)
        g.display.text(font, 'BREAKOUT', 0, 0, st7789.WHITE)
        g.display.rect(90,0, g.max_vol*4+2,6,st7789.WHITE)
        g.display.fill_rect(91,1, g.vol * 4,4,st7789.WHITE)
        g.display.text(font, 'A Start', 10, 16,  st7789.WHITE)
#         if usePaddle :
#             g.display.text(font, 'U Paddle', 0,32,  st7789.WHITE)
#         else :
#             g.display.text(font, 'U Button', 0,32,  st7789.WHITE)
        if demo :
            g.display.text(font, 'B AI-Player', 0,48, st7789.WHITE)
        else :
            g.display.text(font, 'B 1-Player', 0,48, st7789.WHITE)
        g.display.text(font, '  Frame/s {}'.format(g.frameRate), 0,64, st7789.WHITE)
        g.display.text(font, 'Sound FX', 0, 80, st7789.WHITE)
        # g.display.show()
          ...
          while not gameOver :
              ...
              if demo :
                if g.justReleased (g.btnB) :
                  g.display.text(font, 'Demo stopped', 5, 30, st7789.WHITE)
                  # g.display.show()
                  sleep_ms(1000)
                  gameOver = True
                  demoOn = False
