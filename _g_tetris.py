...
size = width, height = 80, 160
color ={'black': st7789.BLACK, 'white': st7789.WHITE}
sqrsize = 8
occupied_squares = []
top_of_screen = (2, 24)
...
def reset_board():
    ...
    occupied_squares = []
    g.display.fill(0)
    g.display.rect(top_x-1, top_y-1, width+2, height+2,st7789.WHITE)

def drawScore () :
  global score, life
  g.display.text(font, 'S:{}'.format (score), 8, 0,st7789.YELLOW)
  g.display.text(font, 'L:{}'.format (life),  48, 0,st7789.YELLOW)
...
def draw_shape():
    ...
    if isinstance(shape_blcks,list):
        for blck in shape_blcks:
            g.display.rect(blck[0], blck[1], sqrsize, sqrsize, st7789.WHITE)
    else:
        g.display.rect(shape_blcks[0], shape_blcks[1], sqrsize, sqrsize, st7789.WHITE)
...
def delete_row(row_no):
    ...
    for sqr in occupied_squares:
        g.display.rect(sqr[x_coord], sqr[y_coord], sqrsize, sqrsize, st7789.WHITE)
...
def move(direction):
    ...
    if legal(shape_blcks):
        for index in range(num_block):
            ...
            g.display.fill_rect(shape_blcks[index][0]-delta_x, shape_blcks[index][1]-delta_y, sqrsize, sqrsize, st7789.BLACK)
        return True
    else:
...
def create_newshape(start_x=board_centre, start_y=32):
    ...
    g.display.fill_rect(82, top_y+32, 60, 40, st7789.BLACK)
    if isinstance(new_shape_blcks, list):
        for blck in new_shape_blcks:
            g.display.rect(blck[0]+60, blck[1]+32, sqrsize, sqrsize, st7789.YELLOW)
    else:
        g.display.rect(new_shape_blcks[0+40], new_shape_blcks[1]+15, sqrsize, sqrsize, st7789.WHITE)
    ...
    if shape_name == 'O':
        ...
        if legal(Rshape_blcks):
            for index in range(num_block): # erase the previous shape
                g.display.fill_rect(shape_blcks[index][0], shape_blcks[index][1],sqrsize, sqrsize, st7789.BLACK)
            return Rshape_blcks
        else:
            return shape_blcks
...
while not exitGame:
  ...
  while True:
    # g.display.fill(0)
    g.display.text(font, 'TETRIS', 0, 0, st7789.WHITE)
    g.display.rect(100,80, g.max_vol*4+2,6,st7789.WHITE)
    g.display.fill_rect(101,81, g.vol * 4,4,st7789.WHITE)
    g.display.text(font, 'A Start', 10, 16,  st7789.WHITE)
    if demo :
        g.display.text(font, '  AI-Player', 10,32, st7789.WHITE)
    else :
        g.display.text(font, '  1-Player', 10,32, st7789.WHITE)
    g.display.text(font, '  Frame/s {}'.format(g.frameRate), 10,48, st7789.WHITE)
    if g.bgm :
        g.display.text(font, 'B Music {}'.format(g.bgm), 10, 64, st7789.WHITE)
    else :
        g.display.text(font, 'B Music Off', 10, 64, st7789.WHITE)
...
#             mov_delay = 10
#             move_dir  = 'down'
#         if g.justPressed(g.btnA | g.btnB) :
#             shape_blcks = rotate()
#             draw_shape()
#             # g.display.show()
#             sleep_ms(r_delay)
#             continue
        if g.pressed(g.btnA) and g.pressed(g.btnB):
            shape_blcks = rotate()
            draw_shape()
            sleep_ms(r_delay)
            continue
        elif g.justPressed(g.btnA | g.btnB):
            move_dir = 'left' if g.pressed(g.btnA) else 'right'
            mov_delay = 50
            move (move_dir)
            draw_shape()
            # g.display.show()
  ...
  if gameOver :
       g.display.fill_rect(20, 120, 90, 35, st7789.RED)
       g.display.text(font, "Game Over", 30, 130, st7789.BLUE, st7789.RED)
       # g.display.show()
       g.playTone('c4', 100)
       g.playTone('e4', 100)
       g.playTone('g4', 100)
